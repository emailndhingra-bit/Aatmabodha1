...
export class PdfUtilsService {
  ...
  generateCompatibilityRing(scores: Record<string, number>): string {
    const rows = Object.entries(scores)
      .map(
        ([k, v]) =>
          `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee;">${this.escapeHtml(k)}</td><td style="padding:6px 10px;text-align:right;border-bottom:1px solid #eee;font-weight:600;">${v}</td></tr>`,
      )
      .join('');
    return `
  ...
  generateScoreTable(koots: { name: string; value: string }[]): string {
    const body = koots
      .map(
        (r) =>
          `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee;">${this.escapeHtml(r.name)}</td><td style="padding:6px 10px;text-align:right;border-bottom:1px solid #eee;">${this.escapeHtml(r.value)}</td></tr>`,
      )
      .join('');
